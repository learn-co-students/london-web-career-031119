...
Authentication
- Logging in (passing credentials to verify access)
- Who are you, and are you who you say you are?


Simplest Form of authentication?
- User model which has a username(or email) and a password
  - Find a user by a submitted username
  - Do a string comparison of the password to the submitted submitted password
  ...
  ```
- Why don't we store passwords in databases?
  - People can steal databases
  - It's a liability!

- We want to obscure passwords
  - We run it through some process that makes it unreadable
  - We use a hashing algorithm
  - On your database, we store not the password, but something called a password_digest

  ```
    def hashing_algo(string)
      # APPPLY HASHING ALGO TO STRING
    end

    user = User.find_by(username: params[:username])

    if user.password_digest == hashing_algo(params[:password])
      # YOU GOOD
    end
  ```

- But! All we need to know is the hashing algorithm that was used
  - Then use brute force to figure out both the hashing algorithim and then use more brute force to then try random passwords with that hashing algorithm
  - Rainbow table attack

- We can take some random string, append it to the password, and then hash that result
  - This is called salting your password
