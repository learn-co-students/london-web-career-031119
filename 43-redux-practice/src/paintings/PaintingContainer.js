...
const mapStateToProps = state => ({
  paintings: state.paintings.filter(painting => painting.museum.name.includes(state.museumFilter)),
  activePainting: state.paintings.find(p => p.id === state.activePaintingId)
});

// props = {
//   fetchPaintings: () => ({ type: 'FETCH_PAINTINGS', payload: [] }),
//   selectPainting: id => ({ type: 'SELECT_PAINTING', payload: id })
// }
